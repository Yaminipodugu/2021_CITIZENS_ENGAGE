...
//    }
    ...
    public CompletionStage<Stream<Complaint>> conditionlist1(String Status,String Category) {
        return supplyAsync(() -> wrap(em -> conditionlist1(em,Status,Category)), executionContext);
    }
    public CompletionStage<Stream<Complaint>> conditionlist2(String Category) {
        return supplyAsync(() -> wrap(em -> conditionlist2(em,Category)), executionContext);
    }
...
//    }
    ...
    private Stream<Complaint> conditionlist1(EntityManager em,String Status,String Category) {

        List<Complaint> complaint = em.createQuery("select p from Complaint p where p.Status =: Status and p.Category=:Category", Complaint.class).setParameter("Status",Status).setParameter("Category",Category).getResultList();
        return complaint.stream();
    }
    private Stream<Complaint> conditionlist2(EntityManager em,String Category) {
